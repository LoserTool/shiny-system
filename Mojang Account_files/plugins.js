...
/*
 * Password strength validation plug-in customized by Kris.
 *
 * Based on: jQuery validate.password plug-in 1.0
 *
...
(function($) {
    ...
    $.validator.passwordRating = function(password, username) {
        ...
        var veryLong = password.length >= 24,
            mediumLong = password.length >= 18,
        ...
        var charCount = countUniqueChars(password),
            highVariation = charCount >= 12;
            mediumVariation = charCount >= 6,
            minVariation = charCount >= 4;

        if (veryLong  && highVariation || lower && upper && digit && minVariation || lower && digits && mediumVariation || upper && digits && mediumVariation || special && minVariation)
            return rating(4, "strong");
        if (mediumLong && highVariation || lower && upper && mediumVariation || lower && digit && minVariation || upper && digit && minVariation)
            return rating(3, "good");
        return rating(2, "weak");
    }

    $.validator.passwordRating.messages = {
        "similar-to-username": "Too similar to e-mail",
        "too-short": "Too short",
        "very-weak": "Very weak",
        "weak": "Weak",
        "good": "Good",
    ...
    $.validator.addMethod("passwordStrength", function(value, element, usernameField) {
        ...
        var explain = meter.find('.password-meter-explain');
        ...
        // update explanation
        if (rating.rate == 0 || rating.rate > 2) {
            explain.hide();
        } else {
            explain.show();
        }
